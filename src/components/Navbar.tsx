...
const Navbar = ({ isOpen, onToggle }: NavbarProps) => {
  ...
  return (
    <>
      ...
      {/* Horizontal Navbar */}
      <nav className={`
        fixed top-0 left-0 right-0 h-20 ${isDark ? 'bg-gray-900/95' : 'bg-white/95'} navbar-blur navbar-shadow z-50 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}
      `}>
        <div className="flex items-center justify-between h-full px-6">
          ...
          <div className={`flex items-center ${isRTL ? 'space-x-reverse' : ''} space-x-3`}>
            ...
            <span className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-800'} ${textAlignClass} tracking-wide hidden sm:block`}>HotelOS</span>
          </div>
          ...
          <div className="hidden lg:flex items-center space-x-3">
            {menuItems.map((item, index) => (
              <Link
                ...
                href={item.href}
                className={`
                  navbar-item flex items-center ${isRTL ? 'space-x-reverse' : ''} space-x-2 px-4 py-2 rounded-lg transition-all duration-200 group
                  ${item.isActive 
          ...
          <div className="flex items-center space-x-2">
            ...
            <button 
              onClick={toggleTheme}
              className={`navbar-button p-2 rounded-lg ${isDark ? 'hover:bg-gray-800/50 text-gray-300 hover:text-yellow-400' : 'hover:bg-gray-100/80 text-gray-600 hover:text-purple-600'} transition-all duration-200`}
            ...
            <button 
              ...
              className={`navbar-button p-2 rounded-lg ${isDark ? 'hover:bg-gray-800/50 text-gray-300 hover:text-blue-400' : 'hover:bg-gray-100/80 text-gray-600 hover:text-blue-600'} transition-all duration-200`}
            ...
            <div className="relative">
              <button 
                ...
                className={`navbar-button flex items-center space-x-2 px-3 py-2 rounded-lg ${isDark ? 'hover:bg-red-900/20 text-gray-300 hover:text-red-400' : 'hover:bg-red-50 text-gray-600 hover:text-red-600'} transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed`}
              >
                <ArrowRightOnRectangleIcon className="w-5 h-5" />
                <span className="hidden sm:block text-sm font-medium">{isLoggingOut ? t('sidebar.loggingOut') : t('sidebar.logout')}</span>
              </button>
            </div>
            ...
            <button
              onClick={onToggle}
              className={`navbar-button lg:hidden p-2 rounded-lg ${isDark ? 'hover:bg-gray-800/50 text-gray-300' : 'hover:bg-gray-100/80 text-gray-600'} transition-all duration-200`}
            >
      ...
      <div className={`
        mobile-menu lg:hidden fixed top-20 left-0 right-0 ${isDark ? 'bg-gray-900/95' : 'bg-white/95'} navbar-blur navbar-shadow border-b ${isDark ? 'border-gray-700' : 'border-gray-200'} z-40 transform transition-transform duration-300 ease-in-out
